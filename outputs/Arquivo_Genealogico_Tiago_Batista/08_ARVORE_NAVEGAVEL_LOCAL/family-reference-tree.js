...
(function () {
  ...
  function render(){
    ...
    for(const route of siblingRoutes){const bus=rowY.get(route.rank)+262+route.lane*12,d=`M${route.left} ${bus}H${route.right}`+route.ids.map(id=>{const g=owner.get(id),start=g.ids.length===1&&g.children.length?254:224;return `M${personPosition(g,id)} ${rowY.get(route.rank)+start}V${bus}`}).join('');strokes.push(`<g class="reference-sibling-link${route.tentative?' tentative-sibling':''}" data-sibling-people="${escape(route.ids.join(' '))}"><title>${route.tentative?'Ligação entre irmãos — a confirmar':'Ligação entre irmãos'}</title><path class="connection-clearance" d="${d}"/><path class="sibling-report" d="${d}"/></g>`);nodes.push(`<span class="reference-sibling-caption${route.tentative?' reference-tentative-caption':''}" style="left:${(route.left+route.right)/2-42}px;top:${bus-7}px">${route.tentative?'a confirmar':'irmãos'}</span>`)}
